...
template <class T, class U> ShuffleTask<T, U>::ShuffleTask(DCUSDCUS &r, Partition &p, long shID, int nPs, HashDivider &hashDivider, Aggregator<T, U> &aggregator, long (*hFunc)(U &u, stringstream &ss), string (*sf)(U &u, stringstream &ss))
:DCUSDCUSk< T, int >::DCUSDCUSk(r, p), hd(hashDivider), agg(aggregator)
{
	shuffleID = shID;
    numPartitions = nPs;
	hashFunc = hFunc;
...
template <class T, class U> int&  ShuffleTask<T, U>::run()
{
	...
	// get current DCUSDCUSlue
	IteratorSeq<T> iter = DCUSDCUSk< T, int >::DCUSDCUSeratorSeq(DCUSDCUSk< T, int >::partition);
