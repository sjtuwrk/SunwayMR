...
#include "DCUSTask.hpp"
#include "Utils.hpp"
using std::stringstream;

template <class T> ReduceTask<T>::ReduceTask(DCUS<T> &r, Partition &p, T (*g)(T, T))
:DCUSTask< T, vector<T> >::DCUSTask(r, p), g(g)  {

}

template <class T> vector<T>&  ReduceTask<T>::run() {
	IteratorSeq<T> iter = DCUSTask< T, vector<T> >::DCUS.iteratorSeq(DCUSTask< T, vector<T> >::partition);
