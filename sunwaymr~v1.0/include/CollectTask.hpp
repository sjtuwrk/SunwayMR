...
template <class T>
CollectTask<T>::CollectTask(DCUSDCUS &r, Partition &p)
:DCUSDCUSk< T, vector<T> >::DCUSDCUSk(r, p)
{
}
...
{
	IteratorSeq<T> iter = DCUSDCUSk< T, vector<T> >::DCUSDCUSeratorSeq(DCUSDCUSk< T, vector<T> >::partition);
//	if(iter.type == 0)
//	{
